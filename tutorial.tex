...
%	\uestccover[<学院名称排版风格>]{<论文题目>}{<学科专业>}{<学号>}{<作者姓名>}{<指导教师>}{<教师职称>}{<学院>} % 单学位学士、研究生通用
	...
	随着智能制造的推进，现代工业系统逐渐变得大型化、复杂化，典型流程工业装置如电源系统中集成了大量不同传感器，其状态变量的采样频率各不相同，统一到同一时间轴后会产生大量结构性缺失值和时间异步现象，给异常检测任务带来较大困难。鉴于大功率供能系统在各工业领域发挥着重大作用，及时对其运行状态进行检测、避免发生严重故障具有重大研究意义。

    逆变器系统是电源系统的关键功率单位，其可靠性为系统的稳定运行提供保障，然而若长期工作在高负载工况下，其内部功率开关、滤波电容、电感等易发生故障，鉴于其状态监测数据呈现多采样率的特点，有必要提出针对多采样率的异常检测方法实现逆变器系统的精准监测。然而针对采样率场景，现有方法大多通过简单插值、降采样或直接忽略缺失值来处理，无法兼顾变量间耦合关系、周期频谱特征与全局时序演化规律的联合建模。为解决该问题，本文提出一种面向多采样率电源系统的自适应图--频率异常检测网络（Adaptive Graph-Frequency Anomaly Detection Network, AGF-ADNet）。该模型引入双向预插补与自监督再遮挡机制，为缺失恢复提供稳定的初始化条件和可靠的监督信号；在插补阶段，构建双专家协同插补模块，其中自适应图重构专家通过动态图学习与图卷积传播挖掘变量间隐式关联，频域重构专家借助快速傅里叶变换与频率注意力增强周期性与谐波信息，再经采样率感知门控融合模块生成最终插补结果；在检测阶段，应用采样率感知 Transformer 全局重构检测器，将变量身份、采样率类别与采样相位编码到统一表示中，联合重构偏差、双专家分歧和分布漂移三类信号构建异常评分，实现插补与检测的协同优化。在三相全桥逆变系统实验平台数据集上的验证结果表明，AGF-ADNet 在测试集上取得了 99.10\% 的准确率、100.00\% 的精确率、98.20\% 的召回率和 99.10\% 的 F1-Score，误报率为 0，优于 CNN、LSTM、VAE、Transformer、MRA-LSTM、Multirate Former和 MR-FA 等对比方法。实验结果验证了本文方法能够缓解多采样率场景下的结构性缺失问题，提升电源系统异常检测的准确性与鲁棒性。
	...
	% 开启英文摘要
	\enabstract
	With the advancement of intelligent manufacturing, modern industrial systems are becoming increasingly large-scale and complex. Typical process-industry installations such as power systems integrate a large number of heterogeneous sensors whose state variables are sampled at different frequencies. After alignment onto a unified timeline, large amounts of structured missing values and temporal asynchrony are introduced, which pose significant challenges to anomaly detection. Since high-power energy supply systems play a vital role across industrial sectors, timely condition monitoring and the prevention of severe faults are of great research significance.

	As a key power unit in a power system, the inverter system is essential for stable system operation. However, when operating under high-load conditions for long periods, internal components such as power switches, filter capacitors, and inductors are prone to faults. Because its condition-monitoring data exhibit multi-rate characteristics, it is necessary to develop anomaly detection methods specifically for multi-rate scenarios to achieve accurate monitoring of inverter systems. Existing methods for multi-rate data usually rely on simple interpolation, down-sampling, or direct omission of missing values, and thus cannot jointly model variable coupling relationships, periodic spectral characteristics, and global temporal evolution patterns. To address this issue, this thesis proposes an Adaptive Graph-Frequency Anomaly Detection Network (AGF-ADNet) for multi-rate power systems. The model introduces a bidirectional pre-imputation strategy and a self-supervised remasking mechanism to provide stable initialization and reliable supervision for missing-value recovery. During the imputation stage, a dual-expert collaborative imputation module is constructed, in which an adaptive graph reconstruction expert exploits dynamic graph learning and graph convolutional propagation to mine implicit inter-variable correlations, while a frequency-domain reconstruction expert leverages fast Fourier transform and frequency attention to enhance periodic and harmonic information. A rate-aware gating fusion module is then employed to generate the final imputation results. During the detection stage, a rate-aware Transformer-based global reconstruction detector encodes variable identity, sampling-rate category, and sampling phase into a unified representation, and integrates reconstruction deviation, expert disagreement, and distribution shift to construct anomaly scores, enabling collaborative optimization of imputation and detection. Experimental results on a dataset collected from a three-phase full-bridge inverter experimental platform show that AGF-ADNet achieves 99.10\% accuracy, 100.00\% precision, 98.20\% recall, and 99.10\% F1-score, with a false alarm rate of 0 on the test set, outperforming CNN, LSTM, VAE, Transformer, MRA-LSTM, Multirate Former, and MR-FA. These results verify that the proposed method can alleviate the structured missingness problem in multi-rate scenarios and improve the accuracy and robustness of anomaly detection for power systems.
    ...
    电源系统及其相关工业场景中的异常往往涉及运行过程中往往涉及化学能、电能与热能之间的复杂转换关系\citess{electronics13153083}以及电压、电流、功率、温度等多类变量的协同变化，既可能体现为瞬态尖峰、缓慢漂移，也可能表现为谐波畸变、频率偏移和波形失真等复杂模式。相比一般静态监测任务，这类系统的异常形态更为多样，单纯依靠人工巡检或固定阈值规则已难以满足实时监测和早期预警的需求。近年来，随着工业现场运行数据的持续积累，数据驱动方法为异常检测提供了新的研究路径。通过挖掘多变量时间序列中的演化规律与变量间的关联结构，即使缺乏完备的机理模型，也能够在一定程度上识别设备健康状态和过程异常。
    ...
    这类方法往往无法适应多采样率环境下的数据特点，可能导致高频变量中的重要动态信息丢失，降低模型对快速变化过程的感知能力，同时引入人为噪声，破坏数据原有的时间依赖结构\citess{Huang2025AttentionBasedMN}。Lu 等人（2024）指出，将多采样率多变量时间序列（Multirate Multivariate Time Series, MR-MTS）强制转换为统一采样率的序列，容易导致信息丢失或结构失真\citess{10313305,LuWS24}。因此，研究者正在探索能够兼顾缺失值填补与采样频率差异的处理方案，以更充分地利用多采样率数据提升故障检测性能。
	...
	\section{本文研究定位}
	基于现有研究基础，本文围绕电源系统多采样率异常检测问题，针对插补不可靠、变量耦合建模不足及周期性异常感知能力弱等核心问题，。首先设计 VAE-Transformer 验证传统方法的插补和检测能力，用掩码感知 VAE 完成缺失值补全，再利用采样率感知 Transformer 进行全局重构检测。在此基础上，进一步提出融合自适应图结构与频域注意力机制的端到端异常检测模型 AGF-ADNet，通过图学习、频域特征增强和门控融合提升模型对变量耦合关系、周期频谱信息和全局时序演化规律的表达能力。
    \section{论文结构安排}
    除本章外，全文其余部分拟安排如下：
    \begin{itemize}
        \item 第二章介绍两类方法共享的多采样率数据表示、样本构造、采样率感知重构检测器和判别流程，并在此基础上阐述 VAE-Transformer 验证模型。
    ...
    \chapter{通用建模框架与VAE-Transformer模型设计}
    ...
    VAE-Transformer 与 AGF-ADNet 均采用先进行缺失插补，再全局重构，最后异常判别的基本范式。设训练或推理阶段模型实际看到的缺失掩码为 $\widetilde{\mathbf{M}}$，则两类方法的共享流程可概括为
    \begin{align}
    ...
    \end{align}
    其中，$\mathcal{I}_{\theta}(\cdot)$ 表示插补器，$\mathbf{X}^{\mathrm{comp}}$ 表示由观测值与插补值共同构成的完整窗口，$\mathcal{D}_{\phi}(\cdot)$ 表示采样率感知 Transformer 重构检测器。VAE-Transformer 使用掩码感知 VAE 作为插补器，用于验证该范式的基本可行性；AGF-ADNet 则将插补器替换为图结构专家与频域专家的门控融合模块，以提升对变量耦合和周期特征的表达能力。
    ...
    \end{equation}
    根据不同步长的取值，可进一步映射为离散采样率类别标识 $r_n$。该元数据在 VAE-Transformer 与 AGF-ADNet 的 Transformer 编码中共同发挥作用；在 AGF-ADNet 中，还进一步用于门控融合模块。

    同时，基于上述推断得到的采样率组，可以对数据进行分块（chunking），并将相似采样率组的数据排列在一起，相似采样率的变量通常具有较强的相关性，可以使模型更稳定地利用同采样层级变量之间的相关信息。数据分块示意如图\ref{fig:data_chunking}所示。其中红色代表缺失样本。
    ...
    \section{通用采样率感知Transformer重构检测器}

    \subsection{设计思想}

    缺失值恢复之后，仍需要从完整窗口中判断样本是否偏离正常运行规律。VAE-Transformer 与 AGF-ADNet 均在插补模块之后引入采样率感知 Transformer 重构检测器。该检测器面向完整窗口 $\mathbf{X}^{\mathrm{comp}}$ 学习正常模式下的全局演化规律，再以重构偏差刻画异常程度。通过先获得较为完整的窗口表示，检测器可以更集中地刻画系统在正常工况下的全局时序结构，而不必直接在高度缺失的输入上同时学习缺失模式与异常模式。
    ...
    在训练分数分布基础上，异常判别阈值可采用高分位数约束的方式确定。
    \begin{equation}
        \theta
        =
        \max
        ...
        \right),
    \end{equation}
    其中，$\kappa=2.0$，$q=0.95$。对于 VAE-Transformer 这类简化验证模型，也可以退化为训练分数均值阈值，以减少额外超参数对方法可行性判断的影响。据此得到初始预测结果
    \begin{equation}
        \hat{y}_t =
        \begin{cases}
    ...
    \section{VAE-Transformer模型设计}

    \subsection{模型定位与总体结构}

    VAE-Transformer 是本文在 AGF-ADNet 之前构建的简化验证模型。该模型不引入自适应图学习、频域重构专家和门控融合机制，而是采用掩码感知变分自编码器（Variational Autoencoder, VAE）完成缺失值补全，再将补全后的完整窗口输入采样率感知 Transformer 进行无监督异常检测。其目的在于验证：即使插补器结构较简单，只要显式利用缺失掩码并在检测阶段引入采样率信息，仍可形成一条可运行的多采样率异常检测链路。

    VAE-Transformer 的主要流程为
    \begin{align}
    ...
    \begin{table}[htbp]
        \centering
        \caption{VAE-Transformer 主要模块及输入输出}
    ...
    该结构的优点是实现简单、训练稳定，能够通过潜变量学习窗口整体分布；其局限也较为明确：编码器和解码器主要由全连接层构成，对变量间显式拓扑关系、局部频谱结构和多尺度动态过程的表达能力有限。因此，VAE-Transformer 更适合作为验证“掩码插补加全局重构”范式的简化模型，而不是最终的增强模型。

    \subsection{仅观测点反向传播的训练目标}

    VAE-Transformer 对缺失值训练的处理参考了 MC-CNN 中改进 BP 算法的思想\citess{song_soft_2025}：可以将其理解为对 BP 神经网络的误差反传过程进行掩码化修改，在反向传播时重构相关损失只由观测点贡献，结构性缺失点的误差项被掩码置零，不参与参数梯度更新。这样可以避免模型把缺失占位值误当作真实监督信号。
    ...
    VAE-Transformer 采用分阶段训练方式。首先训练掩码感知 VAE 插补器；随后固定 VAE 参数，用其生成完整窗口 $\mathbf{X}^{\mathrm{comp}}$，再训练采样率感知 Transformer 检测器。检测器的重构损失同样只在原始可观测位置上计算：
    \begin{equation}
        \mathcal{L}_{\mathrm{det}}^{\mathrm{VT}}
        =
        \frac{\|(\hat{\mathbf{X}}-\mathbf{X}^{\mathrm{comp}}) \odot \mathbf{O}\|_F^2}{\|\mathbf{O}\|_1 + \varepsilon}.
    \end{equation}
    与 AGF-ADNet 的端到端联合优化相比，分阶段训练降低了模块耦合度，便于验证 VAE 插补和 Transformer 检测各自的作用，但也使插补器难以直接接收检测任务的反馈信号。
    ...
    \end{equation}
    此外，模型可选地从补全窗口中提取均值、标准差、一阶差分和低频谱幅值等统计特征，构造分布漂移分数 $s_b^{\mathrm{shift}}$。经训练集分布标准化后，VAE-Transformer 的最终异常分数写为
    \begin{equation}
        s_b^{\mathrm{VT}}
        =
    ...
    本章首先从两类方法中抽取出共同的数据表示、窗口构造、采样率元数据、再遮挡自监督、采样率感知 Transformer 重构检测器以及异常判别流程。在此基础上，进一步给出了 VAE-Transformer 简化验证模型。该模型以掩码感知 VAE 作为插补器，并参考 MC-CNN 的改进 BP 思路，使重构相关损失仅在可观测位置反向传播，从而避免缺失占位值形成错误监督。VAE-Transformer 为后续 AGF-ADNet 的设计提供了基础验证：仅依靠简化插补器能够建立完整检测链路，但对变量耦合关系和频域周期结构的显式利用仍然不足。

    \chapter{AGF-ADNet模型设计}

    \section{模型定位与特化框架}

    在上一章通用框架基础上，AGF-ADNet 面向多采样率电源系统中的变量耦合、周期频谱和全局动态特征进行增强建模。与 VAE-Transformer 相比，AGF-ADNet 不再使用单一全连接 VAE 生成补全结果，而是将插补阶段拆分为自适应图重构专家、频域重构专家和采样率感知门控融合三个部分，并与 Transformer 检测器联合优化。
...
%     \hfill
    ...
    本章在通用多采样率建模框架基础上，围绕 AGF-ADNet 的特化结构与机理，对双向预插补、自适应图学习、图重构专家、频域重构专家、采样率感知门控融合、联合训练目标以及多源异常评分机制进行了系统阐述。与 VAE-Transformer 相比，AGF-ADNet 通过图频双专家和门控融合显式利用变量耦合关系与周期频谱信息，并通过端到端联合优化实现插补与检测之间的协同约束，从而更充分地适应多采样率工业过程中的结构性缺失、变量耦合以及周期动态特征。
    ...
    为全面评价 AGF-ADNet 在多采样率异常检测任务中的判别性能，本文将异常检测问题视为二分类任务，采用准确率、精确率、召回率、故障检测率、误报率和 F1-Score 六项指标。上述指标能够从不同角度刻画模型在正常样本与异常样本上的判别表现，避免单一指标导致的评价片面性。
    ...
    F1-Score 本质上是精确率和召回率的调和平均，因此只有当二者都保持较高水平时，F1-Score 才会取得较大数值。对于多采样率异常检测任务而言，F1-Score 能够较好地反映模型在异常发现能力与报警可靠性之间的综合平衡状态，因此作为最终性能比较中的关键指标之一。

    \section{实验流程介绍}

    实验整体流程如图\ref{fig:flow_chart}所示

    \begin{figure}[!htbp]
        \centering
        \includegraphics[width=0.85\linewidth]{flow_chart.drawio.pdf}
        \caption{实验整体流程}
        \label{fig:flow_chart}
    \end{figure}

    论文将使用的实验数据通过三相全桥逆变系统实验平台进行采集，实验对象为绝缘栅双极型晶体管（Insulated gate bipolar transistor, IGBT）。逆变器系统是一种可以将直流电转换为三相交流电的基础电力电子设备，而IGBT 则是该系统中的关键开关器件，其键合引线、滤波电感和电容等关键部件易发生性能退化，进而引发系统故障。\citess{20241,fei_liu_design_2009}为了模拟真实故障场景，将实际运行状态下可能发生的故障植入到IGBT中，以收集故障样本。为模拟实际工况中常见的电感器匝间短路故障，实验设计中通过在滤波电感的两端并联一个可调电阻器，以复现此类故障特征并采集相应的故障状态样本。获取得到的数据集包含每项六个基于电压测量得到的特征，合计十八个特征，分别包含：平均值$\bar{V}$、峰峰值$V_{pp}$、均方根值$V_{rms}$、频率$f$、裕度$M_v$和峭度$K_v$。
    ...
    \section{实验结果}
    传统模型，包括 CNN、LSTM、VAE 和 Transformer 的可视化检测结果如图所示。
    \begin{figure}[!htbp]
    \centering
    \subfloat[CNN 检测结果]{
  ...
  \textbf{FAR} & \textbf{F1-Score} \\
    ...
    \begin{axis}[
        ...
        ylabel={Accuracy/Precision/FDR/F1-Score (\%)},
        ymin=0,
        ymax=100,
        ymajorgrids=true,
    ...
    MC-CNN、MRA-LSTM、Multirate Former 、MR-FA 等前沿方法检测结果如图\ref{fig:new_baseline_comparison}所示。与传统模型相比，这些方法的检测效果有所改善，但从整体结果来看召回率和误报率仍存在改进空间。
    ...
    前沿建模方法的性能指标如表\ref{tab:new_model_metrics}所示。MR-FA 在对比方法中表现最优，Accuracy、Recall 和 F1-Score 分别为 86.92\%、77.20\% 和 85.52\%；MRA-LSTM 紧随其后，整体指标也保持在较高水平。Multirate Former 的表现处于中等水平。MC-CNN 则表现最差，其 Recall 仅为 3.35\%，并不具备检测能力
  ...
  \end{table}
    ...
    本文所提出方法的可视化检测结果如图\ref{fig:agf-adnet_fig}所示，各项性能指标如表\ref{tab:agf_adnet_model_metrics}所示。从定量结果看，AGF-ADNet 在测试集上取得了 99.10\% 的准确率、100.00\% 的精确率、98.20\% 的召回率、0.00\% 的误报率以及 99.10\% 的 F1-Score。相较于表现最优的对比方法 MR-FA，准确率提升了 12.18 个百分点，召回率提升了 21.00 个百分点，F1-Score 提升了 13.58 个百分点，同时将误报率由 3.35\% 降至 0.00\%。精确率达到 100.00\% ，召回率达到 98.20\% 。上述结果表明，所提方法不仅显著提升了异常检测的灵敏度，也兼顾了误报抑制能力，在综合性能上明显优于现有对比模型。

  \begin{table}[!htbp]
      \caption{AGF-ADNet 分类指标}
  ...
  \end{table}
    ...
    \begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{agf-adnet.png}
    \caption{AGF-ADNet 检测结果}
    \label{fig:agf-adnet_fig}
  \end{figure}

    为了更直观地比较各模型在核心分类指标上的整体分布特征，进一步选取 Accuracy、Precision、FDR 和 F1-Score 四项指标绘制箱形图，如图\ref{fig:new_model_boxplot}所示。可以看出，AGF-ADNet 的箱体整体位于最高区间，中位数和上下四分位数均明显优于其余对比方法，说明其综合性能最优且稳定性最好；MR-FA 与 MRA-LSTM 构成第二梯队，整体表现接近但仍与本文方法存在明显差距；Multirate Former 的箱体位置居中，说明其检测能力有一定提升但总体水平仍有限；MC-CNN 的箱体集中在低值区间，且下须显著下探，进一步反映其在 FDR 和 F1 指标上的明显短板。
    ...
    \begin{axis}[
        ...
        grid style={dashed, gray!35},
        xtick={1,2,3,4,5},
        xticklabels={MC-CNN,MRA-LSTM,Multirate Former,MR-FA,AGF-ADNet},
    ...
    为更加直观的比较传统方法、前沿方法与本文方法在核心指标上的整体表现，进一步绘制 9 种方法的拆分雷达图，如图\ref{fig:all_methods_radar}所示。各子图采用完全一致的半径刻度范围，便于直接比较不同方法在四项核心指标上的整体包络形态。可以看出，AGF-ADNet 在四个维度上均接近最外圈，整体面积最大，说明其综合性能最优；MR-FA 与 MRA-LSTM 构成第二梯队；VAE、Transformer 与 Multirate Former 处于中间水平；CNN、LSTM 和尤其是 MC-CNN 的雷达区域明显收缩，表明其在多采样率场景下的异常检测能力仍存在较大局限。
    ...
    综合分析认为，本文方法性能提升的原因主要体现在三个方面：其一，双向预插补与自监督再遮挡机制改善了多采样率对齐后缺失值恢复的初始质量，为后续检测提供了更可靠的数据基础；其二，自适应图重构专家与频域重构专家分别从变量耦合关系和周期频谱结构两个角度补充信息，使模型能够同时感知跨变量关联异常与谐波、振荡类异常；其三，采样率感知 Transformer 与多源异常评分机制增强了模型对长程依赖、缓慢漂移以及局部突变等多类型异常的统一判别能力。因此，AGF-ADNet 在复杂电源系统多采样率场景下表现出更高的检测精度和更好的工程应用潜力。
    ...
    \subsection{实验设置}
    为验证 AGF-ADNet 中各组成模块的有效性，本文在相同训练集与测试集上设置消融实验。训练数据为正常工况样本，测试数据为故障样本；检测阈值均由训练集异常分数确定。评价指标包括 Accuracy、Precision、Recall、FDR、FAR 和 F1-Score。完整模型结果采用前文 AGF-ADNet 分类实验结果，消融模型结果来自下各实验输出。
    ...
    因此，在当前证据下，建议继续采用默认配置 \(\texttt{seq\_len}=50\)、\(\texttt{holdout\_ratio}=0.15\)、\(\texttt{detector\_d\_model}=128\)、\(\texttt{detector\_layers}=3\)、\(\texttt{gcn\_hidden\_dim}=32\) 作为后续主实验设置。
        ...
        本文围绕电源系统多采样率过程异常检测问题开展研究，针对多源监测变量采样频率不一致所引起的结构性缺失、时间异步、变量耦合关系难以表达以及周期频谱信息利用不足等关键挑战，先构建 VAE-Transformer 作为验证模型，再提出面向多采样率电源系统的自适应图--频率异常检测网络 AGF-ADNet。论文以三相全桥逆变系统为应用背景，从问题建模、模型设计、实验验证和机制分析等方面进行了较为系统的研究，主要工作与结论可概括如下。

        第一，围绕多采样率工业数据的基本特征，构建了两类方法共享的异常检测问题描述与数据处理流程。本文通过缺失掩码生成、仅基于观测位置的标准化、滑动窗口构造、采样率元数据推断以及自监督再遮挡等步骤，将原始多采样率监测序列转化为适合统一建模的窗口样本表示，为后续插补与检测协同优化奠定了数据基础。该表示方式能够显式保留何处缺失、何处观测的结构信息，同时为采样率感知建模提供必要先验。

        第二，设计了 VAE-Transformer 简化验证模型。该模型以掩码感知 VAE 作为插补器，将观测值和观测掩码共同输入编码器，并参考 MC-CNN 的改进 BP 思想，使重构相关损失仅在原始可观测位置反向传播，避免缺失占位值形成伪监督；随后利用采样率感知 Transformer 对补全窗口进行全局重构检测。

        第三，在VAE-Transformer的基础上，针对多采样率场景下缺失恢复不可靠与异常表征不足的问题，设计了 AGF-ADNet 模型。模型在插补阶段引入双向预插补与自监督再遮挡机制，为学习型缺失恢复提供稳定初始化与有效监督；进一步构建自适应图重构专家与频域重构专家，分别从跨变量依赖关系和周期频谱结构两条路径补充信息，并通过采样率感知门控融合模块生成最终插补结果。在检测阶段，模型利用采样率感知 Transformer 对完整窗口进行全局重构，同时融合重构偏差、双专家分歧与分布漂移三类信号构建异常评分，实现了插补与检测的一体化协同建模。

        第四，基于三相全桥逆变系统实验平台采集数据，对所提方法进行了系统验证。实验数据包含 18 个人工特征，并通过降采样构造多采样率变量场景。与 CNN、LSTM、VAE、Transformer、MC-CNN、MRA-LSTM、Multirate Former 以及 MR-FA 等方法相比，AGF-ADNet 在测试集上取得了 99.10\% 的 Accuracy、100.00\% 的 Precision、98.20\% 的 Recall、98.20\% 的 FDR、0.00\% 的 FAR 和 99.10\% 的 F1-Score。相较于性能最优的对比方法 MR-FA，所提方法的 Accuracy 提升了 12.18 个百分点，Recall 和 FDR 提升了 21.00 个百分点，F1-Score 提升了 13.58 个百分点，同时将误报率降低至 0，表明该方法在异常检出能力与误报抑制能力之间实现了更优平衡。

        第五，结合消融实验与参数敏感性实验，对模型有效性来源进行了进一步分析。消融结果表明，完整 AGF-ADNet 在所有消融设置中取得最优性能，说明双专家插补、门控融合、采样率感知检测器以及多源异常评分之间存在明显协同作用。图频门控 + Transformer 是最接近完整模型的消融版本，F1-Score 达到 0.9250，但与完整模型相比仍存在召回率差距，表明采样率感知建模、相位信息和分布漂移分数对故障窗口覆盖能力具有进一步增益。单专家结果显示，仅图专家 + Transformer 明显优于仅频域专家 + Transformer，说明当前故障场景中变量间耦合关系是更主要的判别信息来源；而图频门控 + 重构输出和两个单专家直接重构输出的结果则表明，仅依赖插补重构误差难以形成稳定连续的异常响应，Transformer 检测器对于最终检测性能具有关键作用。参数敏感性结果表明，序列窗口长度是最关键的敏感因素，过短窗口会显著削弱异常片段的连续检出能力；相比之下，再遮挡比例、检测器宽度与层数在当前取值范围内表现出较好的鲁棒性，GCN 分支隐藏维度则呈现较明显的中间最优特征。

        综合来看，VAE-Transformer 验证了先进行插补并使用 Transformer 检测具有可行性，而 AGF-ADNet 进一步通过图频双专家与多源评分机制提升了缺失恢复和异常判别能力。本文所提出的 AGF-ADNet 能够较好地缓解多采样率场景下的结构性缺失问题，并充分利用变量耦合关系、频谱特征与全局时序演化信息，在电源系统异常检测任务中表现出较高的检测精度、较强的鲁棒性以及一定的工程应用潜力。
        ...
        第二，面向在线监测与边缘部署需求，进一步优化模型效率。当前模型采用双专家插补与 Transformer 全局重构相结合的结构，在实时性、模型规模和部署成本方面仍有优化空间。后续可探索构建更轻量化、适用于在线诊断、嵌入式或边缘设备的多采样率异常检测框架。
        ...
        首先，我必须特别感谢我的老师王敏和邱根。在本次研究和论文撰写的过程中，两位老师严谨的治学态度和渊博的学识让我受益匪浅。他们为我提供了非常宝贵且专业的指导意见，指引我不断拓宽思路、完善细节。同时，我要向我的师兄王思贻表达深深的感激，从论文的初期准备到最终定稿，他为我提供了全程的跟进与细致的指导，倾注了大量的心血，帮助我克服了诸多困难。
